...
*- These will be run at different times throughout the **preparation** and **procedure**.*
 - *Shell Scripts List*
	...
	-  (b) [merge_xml.sh](../blob/master/workingDir/shell_scripts/merge_xml.sh) - merges all *.xml files contained within this directory. This is useful after transformation because it is easier to check if a specific element is available in every file.  **Run this file last**
	 - \(c\) [correct-invalid-chars.sh](/json-to-mods-xslt/blob/master/workingDir/add-data-tags-and-cleanup-html.sh) - this corrects several of the invalid characters found in the original treesearch metadata. It should  be run after **(a)** and before **(b)**. 
